...
export function SessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo(() => ({ token, user, isRestoring, signIn: async (userId: string, password: string) => { const session = await login(userId.trim(), password); await saveSession(session.token, session.user); setToken(session.token); setUser(session.user); registerForPushNotifications().catch(() => {}); return session.user; }, signOut: async () => { await clearSession(); setToken(null); setUser(null); } }), [token, user, isRestoring]);
